...
public class JPanelManagementTable extends javax.swing.JPanel {
    ...
    protected HashMap<Integer, JPanelTableRowItem> listJPDRI = new HashMap<>();
    public JPanelManagementTable() {
        ...
        while(i.hasNext()){
            ...
            listJPDRI.put(id, jpTRI);
            jPanelTableContent.add(jpTRI);
            count++;
        };
        int height = listTable.getData().size() * 62;
    ...
    private void filter(){
        ...
        while(i.hasNext()){
            ViewItem t = (ViewItem) i.next();
            Integer id = (Integer) t.get("MaBan");
            jPanelTableContent.add(listJPDRI.get(id));
        };
        int height = listTable.getFilterData().size() * 62;
        jPanelTableContent.setPreferredSize(new Dimension(780, height));
        this.revalidate();
        this.repaint();
    }
